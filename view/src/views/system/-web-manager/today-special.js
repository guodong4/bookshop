...
import SetSpecial from "./set-special";
const Index = {
    data() {
        return {
            dataSource: [],
    ...
    methods: { 
        ...
        add() {
            this.$refs.setspecial.$children[0].show();
        },
        update(id){
            this.$refs.setspecial.$children[0].show(id);
        },
        delete(id) {
            $ajax({
                url: "/special/delete",
    ...
    render() {
        ...
        return <div>
            ...
            <SetSpecial ref="setspecial" reload={this.reset}/>
        </div>
    }
};
